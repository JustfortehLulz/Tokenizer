...
public:
	...
	shared_ptr_to_constant ofTypeCreateConstant(shared_ptr_to_constant constToCreateFromSP)
	{
		...
		case 'i': // int
		case 'c': // char
			return constToCreateFromSP->putValueIntoInt();// cases for characters and Floats
		case 'f': // float
...
public:
	...
	shared_ptr_to_constant eval()
	{
		shared_ptr_to_constant eval_exp = 0;
		auto strVal = getStringValue();
		auto lefTkn = evalChildToken(0);
		auto righTkn = evalChildToken(1);
		auto key = getChildToken(0)->getStringValue();
		auto mapEntry = evalMap[key];
		/*auto holdL = lefTkn;
		auto holdR = righTkn;
		std::cerr <<"Value in leftTkn = " << holdL << " and right = " << holdR << "\n";*/
		if (strVal =="=")//check if value is '=' ****** this works (id a = b)
			eval_exp = righTkn;
		else if(strVal == "+=")//check for +=
			eval_exp = lefTkn->add(*righTkn);
		else if(strVal == "-=")//run -=
			eval_exp = lefTkn->sub(*righTkn);
		else if(strVal == "/=") // run /=
			eval_exp = lefTkn->div(*righTkn);
		return evalMap[key].sptc = mapEntry.ts.ofTypeCreateConstant(eval_exp);
...
//		auto holdR = righTkn;
//		std::cerr <<"Value in leftTkn = " << holdL << " and right = " << holdR << "\n";
		//shift n bits to the left or the right
		if(strVal == "<<=")// check if the operator is left shift '<<=' example: a <<= b --> a = a << b
			eval_exp = lefTkn-> leftShift(*righTkn);
		else if(strVal == ">>=")// check if the operator is a right shift '>>=' example: a >>= b --> a = a >> b
			eval_exp = lefTkn-> rightShift(*righTkn);
		else if(strVal =="%=") // run modulo assignment operator '%=' example: a %= b --> a = a % b
			eval_exp = lefTkn-> mod(*righTkn);
		return evalMap[key].sptc = mapEntry.ts.ofTypeCreateConstant(eval_exp);
	};
...
public:
	...
	shared_ptr_to_constant eval()
	{//implement '?' if false evaluate after ? if true evaluate after :
		shared_ptr_to_constant eval_exp = 0;
		auto lefTkn = evalChildToken(0);
		auto midTkn = evalChildToken(1);
		auto righTkn = evalChildToken(2);
		if(lefTkn -> asBool())
			eval_exp = midTkn;
		else
			eval_exp =righTkn;
...
public:
	...
	shared_ptr_to_constant eval()
	{//implement '<<' & '>>'
		shared_ptr_to_constant eval_exp;
		auto strVal = getStringValue();
		auto righTkn = evalChildToken(1);
		auto lefTkn = evalChildToken(0);
		if (strVal == "<<")//run left shift
			eval_exp = lefTkn-> leftShift(*righTkn);
		else//run right shift
			eval_exp = lefTkn-> rightShift(*righTkn);
		return eval_exp;
	}
};
...
public:
	...
	{//implement '+' & '-'
		...
		//implement'+'
		if(strVal=="+")
			eval_exp = evalChildToken(0)->add(*righTkn);
		else//implementing '-'
			eval_exp = evalChildToken(0)->sub(*righTkn);
		return eval_exp;
	}
};
...
public:
	...
	shared_ptr_to_constant eval()
	{//implementation of postfix operators '++' | '--'
		shared_ptr_to_constant eval_exp = 0;
		if (getStringValue()=="++")
			eval_exp = evalChildToken(0)-> postfix_increment();
		else
			eval_exp = evalChildToken(0)-> postfix_decrement();
		return eval_exp;
	}
};
...
public:
	...
	shared_ptr_to_constant postfix_decrement()// postfix decrement logic is identical to increment but with --
	{ // a --;
		T temp = value;
		value--;
		return create(temp);
...
public:
	...
	{// ***** implementing '~'
		return create(~this->getIntValue());
	}
	shared_ptr_to_constant mod(constant& right)
	{
		auto val = this->getValue();
		if(right.getIntValue())
			return create(val % right.getIntValue());
		else
			throw(std::overflow_error("Mod by zero"));
	}
	shared_ptr_to_constant leftShift(constant& right)
	{// ***** implementing '<<'
		return create(this->getIntValue() << right.getIntValue());
	}
